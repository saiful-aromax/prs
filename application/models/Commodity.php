 ...
 * Time: 3:57 PM
 */
class Commodity extends MY_Model
{
    ...
    function get_list($offset, $limit, $cond)
    {
        if ($offset == 0 && $limit == 0) {
            ...
            $this->db->select('commodities.*');
        }

        $this->db->from('commodities');
        if (!empty($cond['name'])) {
            $where = "( commodities.name LIKE '%{$cond['name']}%')";
            $this->db->where($where);
        }
        ...
        $this->db->order_by('commodities.name', 'ASC');
        $query = $this->db->get();
        $query->result();
        return $query->result();
    }
    ...
    function add($data)
    {
        return $this->db->insert('commodities', $data);
    }
...
//    }
    ...
    {
        return $this->db->update('commodities', $data, ['id' => $id]);
    }
    ...
    function delete($id)
    {
        return $this->db->delete('commodities', ['id' => $id]);
    }
