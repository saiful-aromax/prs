<?php
if ($report_type == 'HTML') { ?><div class="box box-info"><?php } ?>
    <div class="box-body table-responsive" data-pattern="priority-columns">
        ...
        <table class="table table-bordered table-striped transaction_form">
            ...
            <tr>
                ...
                <?php if ($reporting_periods_info['name'] != 'Annual') { ?>
                    <th>
                        Annual Target of <?php echo $years_info['name'] . ' '; ?>
                    </th>
                <?php } ?>
                <th>
                    FY <?php echo $years_info['name'] . ' ';
                    echo $reporting_periods_info['name']; ?> Target
                </th>
                <th>
                    FY <?php echo $years_info['name'] . ' ';
                    echo $reporting_periods_info['name']; ?> Result
                </th>
                <th>
                    Deviation (%)
                </th>
                ...
                <?php if ($years_next1['name']) { ?>
                    <th>
                        FY <?php echo $years_next1['name'] ?> Target
                    </th>
                <?php } ?>
                <?php if ($years_next2['name']) { ?>
                    <th>
                        FY <?php echo $years_next2['name'] ?> Target
                    </th>
                <?php } ?>
                <?php if ($years_next3['name']) { ?>
                    <th>
                        FY <?php echo $years_next3['name'] ?> Target
                    </th>
                <?php } ?>
                <th>
            ...
            foreach ($transactions as $row) { //echo $row['cluster_rowspan']; die;
                ...
                <tr>
                    ...
                    <?php }
                    $target_period_value = $row['transaction']['Target_' . strtolower(str_replace(" ", "_", $reporting_periods_info['name']))];
                    $result_period_value = $row['transaction']['Result_' . strtolower(str_replace(" ", "_", $reporting_periods_info['name']))]; ?>
...
<?php if ($report_type == 'HTML') { ?>
    <div class="col-sm-offset-10">
        <button type="submit"
                class="btn btn-success submit_buttons positive"><?php echo $is_authorization == 0 ? 'Un-Authorize' : 'Authorize'; ?></button>
    </div>
    </div><?php } ?>